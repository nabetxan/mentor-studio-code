...
An AI-mentor extension for VS Code that can wire Mentor into Claude Code and Codex. Track your understanding, spot your weak points, and keep learning in your own projects.

> **Note:** Mentor can be wired into Claude Code and Codex. If you use [Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview), please refer to [Anthropic's official documentation](https://docs.anthropic.com/en/docs/claude-code/overview) for installation, requirements, and available plans.
>
...
4. Choose which AI tools should use Mentor: `Claude Code` and/or `Codex`
5. If you enable Claude Code, choose where to add the mentor reference — project-wide `CLAUDE.md` or your personal settings
6. If you enable Codex, Setup writes a managed Mentor block to project `AGENTS.md`
7. When the "Reload Window" dialog appears, click it
8. After reload, the dashboard opens automatically

You can later change provider wiring from **Settings** → **AI Tools Using Mentor**. If no provider is enabled, the Settings tab shows a warning and Mentor cannot be turned on until Claude Code or Codex is configured.
...
Each button copies a prompt to your clipboard. Paste it into Claude Code or Codex to start the corresponding mentor session.
...
| Setting    | Description                                                                                               |
| ---------- | --------------------------------------------------------------------------------------------------------- |
| AI Tools Using Mentor | Enable Claude Code and/or Codex, and choose whether Claude Code uses project or personal `CLAUDE.md` wiring |
...
Claude Code や Codex に連携できる、AIメンター付きの VSCode 拡張。自分のプロジェクトの中で、理解度の確認・弱点の可視化・復習ができます。

> **注:** Mentor は Claude Code と Codex に連携できます。[Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview) を使う場合は、インストール・動作要件・利用可能なプランについて [Anthropic公式ドキュメント](https://docs.anthropic.com/en/docs/claude-code/overview) をご確認ください。
>
...
4. Mentor を使う AI ツールとして `Claude Code` と `Codex` から利用したいものを選ぶ
5. Claude Code を有効にした場合は、メンター参照の追加先を選択する — プロジェクト共通の `CLAUDE.md` または個人設定
6. Codex を有効にした場合は、プロジェクトの `AGENTS.md` に管理対象の Mentor ブロックが追加される
7. 「Reload Window」ダイアログが表示されたら押す
8. リロード後、ダッシュボードが自動で開く

あとから **Settings** → **AI Tools Using Mentor** で変更できます。どの AI ツールも有効になっていない場合は Settings タブに警告が表示され、その状態では Mentor を有効化できません。
...
各ボタンを押すとプロンプトがクリップボードにコピーされます。Claude Code または Codex に貼り付けて、対応するメンターセッションを開始します。
...
| 設定項目   | 内容                                                                                                      |
| ---------- | --------------------------------------------------------------------------------------------------------- |
| Mentor機能を利用するAIツール | Claude Code / Codex の有効化と、Claude Code 用 `CLAUDE.md` の設定先（プロジェクト / 個人）の切り替え |
